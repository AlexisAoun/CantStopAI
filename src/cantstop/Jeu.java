...
public class Jeu {
    ...
    private int[] maximum;
    // le positionnement des bonzes [numéro de colone 2 à 12][avancement sur la
    // colone]
    private int[][] bonzes;
    ...
    private int[] des;
    // les possibilités de regroupement + infos utiles: [somme d'une paire de
    // dé][indication sur la possibilité de choisir cette somme]
    ...
    public void init() {
        ...
        strats = new Strategie[nbJoueurs];
        ...
        for (int i = 0; i < nbJoueurs; i++) {
            ...
            joueurs[i].status = clavier.getIntBetween(0, 1);
            if (joueurs[i].status == 1) {
                System.out.println("Numéro de la stratégie à charger");
                int s = clavier.getIntBetween(0, 500);
                try {
                    strats[i] = (Strategie) Class.forName("strategies.Strat" + s).newInstance();
                } catch (Exception e) {
                    System.exit(-1);
                }
            }
    ...
    public void jeterLesDes() {
        ...
        // Determination des possibilités d'avancement + indication sur les choix
        // possible suivant l'état du jeu
    ...
    public void construireChoix() {
        ...
        int tmp;
        if (choixPossible) {
            // Pour chaque combinaison de 2 paires de dé on regarde détermine les choix
            // possibles suivant le nb de bonzes restants
            for (int i = 0; i < 3; i++) {
    ...
    public int strategieChoix() {
        return strats[actif].choix(this);
    }

    /**
     * Demande de decision du joueur pour continuer son tour ou arreter et donc
     * validé l'avancement de ses bonzes
     */
    public boolean stop() {
        // Si joueur humain on demande de decider
        boolean res = false;
    ...
    /**
     * Valide l'avancement des bonzes du joueur actif et ajustement de son score si
     * nécessaire
     */
    ...
    /**
     * Test la possibilité d'avancement sur la colone "valeur" suivant l'état actuel
     * du jeu
     * 
     * @param valeur la colonne à tester
     * @return 0 si pas possible, 1 si c'est possible sur une colone ou un bonze est
     *         déja présent, 10 si c'est possible avec un nouveau bonze
     */
    public int possible(int valeur) {
        int res = 0;
        // Si il nous reste des bonzes et que les colonnes ne sont pas fini (validé ou
        // non)
        ...
        // Si il ne reste pas de bonze on vérifie que la colones n'est pas bloqué et
        // qu'on a déjà un bonze dessus
        // Si c'est le cas on retourne 1
        else if ((bonzes[0][0] == valeur || bonzes[1][0] == valeur || bonzes[2][0] == valeur) && !bloque[valeur - 2]
                && !fini[valeur - 2]) {
            res = 1;
            choixPossible = true;
        }
            ...
            if (!premierCoup) {
                ...
                if (stop()) {
                    ...
                    if (!end) {
                        ...
                        System.out.print("                                 ");
                        System.out.println(
                                joueurs[(actif + nbJoueurs - 1) % nbJoueurs].pseudo + " a validé son avancement.");
                        System.out.print("                                 ");
        ...
        try {
            ...
            for (int a = 0; a < groupes.length - 1; a++) {
                for (int b = a + 1; b < groupes.length; b++) {
                    ...
                    strats[0] = (Strategie) Class.forName("strategies.Strat" + groupes[a]).newInstance();
                    strats[1] = (Strategie) Class.forName("strategies.Strat" + groupes[b]).newInstance();
                    ...
                    System.out.println(" => " + victoire[0][0] + " - " + victoire[1][0] + " (" + victoire[0][2] + ", "
                            + victoire[1][2] + ")");
            ...
            for (int i = 0; i < groupes.length; i++) {
                ...
                System.out.println(tmp.getName() + ": " + res[i][0] + " " + res[i][1] + " " + res[i][2] + " "
                        + res[i][3] + " " + res[i][4]);
            }
        } catch (Exception e) {
    ...
    /**
     * Permet de récupérer le tableau indiquant le status fini (vrai ou faux) de
     * chaque voie
     * 
     * @param aucun
    ...
    /**
     * Permet de récupérer le tableau indiquant le status bloqué (vrai ou faux) de
     * chaque voie
     * 
     * @param aucun
